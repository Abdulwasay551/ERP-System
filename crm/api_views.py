...
class CustomerViewSet(IdempotentCreateMixin, PkConflictReportingMixin, SoftDeleteViewSetMixin, viewsets.ModelViewSet):
    ...
    def ledger_pdf(self, request, pk=None):
        ...
        if request.query_params.get('extended') == 'true':
            ...
            buf = build_extended_ledger_pdf(customer.name, 'Customer', entries, details, date_from, date_to, closing_balance)
            filename = f'{customer.name}-ledger-extended.pdf'
        else:
            buf = build_ledger_pdf(customer.name, 'Customer', entries, date_from, date_to, closing_balance)
            filename = f'{customer.name}-ledger.pdf'
    ...
    def ledger_entry_detail(self, request, pk=None, entry_id=None):
        """The real document behind one ledger row (invoice line items w/ tracking, or
        ...
        customer = self.get_object()
        try:
            entry = customer.ledger_entries.get(pk=entry_id)
