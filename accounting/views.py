...
from .models import Account, AccountGroup, AccountCategory, Journal, JournalEntry, JournalItem
...
def api_financial_reports(request):
    ...
    try:
        ...
        if report_type == 'balance_sheet':
            return generate_balance_sheet_data(company)
        elif report_type == 'income_statement':
            return generate_income_statement_data(company)
        elif report_type == 'cash_flow':
            return generate_cash_flow_data(company)
        elif report_type == 'trial_balance':
            return generate_trial_balance_data(company)
        else:
            return JsonResponse({'success': False, 'error': 'Invalid report type'}, status=400)
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)}, status=500)

def generate_balance_sheet_data(company):
...
def generate_income_statement_data(company):
...
def generate_cash_flow_data(company):
    """Generate cash flow statement data"""
    try:
        # This is a simplified cash flow statement
        # In a real implementation, you'd need more complex logic to categorize cash flows
        ...
        return JsonResponse({
            ...
            'data': {
                'operating_activities': [
                    {'description': 'Net Income', 'amount': 485000},
                    {'description': 'Depreciation', 'amount': 45000},
                    {'description': 'Working Capital Changes', 'amount': -25000}
                ],
                'investing_activities': [
                    {'description': 'Equipment Purchase', 'amount': -150000}
                ],
                'financing_activities': [
                    {'description': 'Loan Proceeds', 'amount': 100000},
                    {'description': 'Dividends Paid', 'amount': -330000}
                ],
                'totals': {
                    'operating': 505000,
                    'investing': -150000,
...
def generate_trial_balance_data(company):
